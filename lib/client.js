// Client bundle — dsh.client 通道（与 src/client.js 同一函数体）
// 说明：当前函数体面向 DSH 动态 client 运行器（React/styles/host 由运行器注入）。
//       静态 bundle 的 client 通道需经官方 client preset 构建（TODO，见 README「插件市场发布」）。
// Client 半区（右侧悬浮直播窗口 UI）
...
`)
    ...
    const LiveWindow = () => {
      ...
      React.useEffect(() => {
        ...
        const refresh = async () => {
          try {
            const res = await host.call('stream/status')
            if (!alive) return
            applySnap(res)
        ...
        refresh()
        const disposer = ctx.timer.interval(refresh, 1000)
        return () => { alive = false; mountedRef.current = false; disposer(); releaseMedia(); stopScreenStream() }
      }, [])
      ...
      const setTargetsAndSync = async (ids) => {
        ...
        let res = null
        try {
          res = await host.call('stream/setTargets', { ids })
        } catch (e) { return }
        applySnap(res)
          ...
          if (videoTrack) {
            videoTrack.addEventListener('ended', () => {
              ...
              if (modeRef.current === 'screen' && latestSnap && latestSnap.activeCount > 0) {
                ...
                cropRef.current = null
                setPipOnBoth(false)
                host.call('stream/setTargets', { ids: [] }).then(applySnap).catch(() => {})
              }
            })
          }
        } catch (e) {
      ...
      const setEngineMode = async (m) => {
        try {
          const res = await host.call('stream/setPushMode', { mode: m })
          applySnap(res)
        } catch (e) {}
      }
      ...
      const installFfmpeg = async () => {
        ...
        try {
          const res = await host.call('ffmpeg/install')
          setFfInfo(res)
        } catch (e) {}
        setInstallingFfmpeg(false)
        try {
          const res = await host.call('ffmpeg/status')
          setFfInfo(res)
        } catch (e) {}
      }
      ...
      const saveSettings = async () => {
        ...
        try {
          const res = await host.call('stream/setConfig', { id: selPlatform, server, streamKey })
          applySnap(res)
      ...
      const copyAddress = async (ep) => {
        ...
        try {
          if (typeof navigator !== 'undefined' && navigator.clipboard && navigator.clipboard.writeText) {
            ...
            setCopied(ep)
            ctx.timer.timeout(() => setCopied(''), 1500)
          }
        } catch (e) {}
      }
